...
\item Keine Datenbank-Anforderungen an den Server gestellt werden müssen
\end{itemize}

Der Umstand, dass keine Datenbank zum Einsatz kommt ist auch unter Gesichtspunkten des gleichzeitigen Zugriffs vertretbar. Es gibt nur eine Datei (Data\textbackslash KlientenName \textbackslash AssistanceInput \textbackslash YYYY-m.txt), bei der theoretisch mehrere Benutzer auf einmal schreiben können. Dies wurde verhindert, in dem eine Art Semaphore eingesetzt wurde. Unter PHP gibt es keine echte Semaphoren, aber es gibt die Möglichkeit auf eine Datei sperrend zuzugreifen.
%TODO Konzept der Lock-Datei erleutern
%(Data\textbackslash KlientenName \textbackslash AssistanceInput \textbackslash YYYY-m_lock.txt)

\section{Übersicht}
* alle verfügbaren Funktionen werden nutzerabhängig dargestellt
* 
...
* pro Tag
   ...
   * private Notizen
   * öffentliche Notizen
* pro Monat: Nachricht an Team
* Team benachrichtigen

\section{Standard Dienstzeiten}
* Möglichkeit anzugeben, wie normalerweise die Arbeistzeiten sind
...
Mit Hilfe des Kalenders kann ein Assistent die Termine eintragen, an denen er Dienst machen kann. Durch einen Klick auf ein Datum ändert sich der Zustand des Datums und damit auch die Hintergrundfarbe.
...
%TODO Screenshot vom Anlegen eines neuen Klienten
...
\section{Navigation und Monatsnavigation}
* Navgiation immer gut sichtbar oben links
